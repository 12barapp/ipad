...
-(id<AtkDragSourceProtocol>)getDataSource{
...
- (void)start:(UIView *)rootView recognizerClass:(Class)recognizerClass
{
    ...
    if(rootView)
    {
        ...
        [self.recognizer setDelaysTouchesBegan:YES];
        [self.rootView addGestureRecognizer:self.recognizer];
        
     /*   self.recognizer.enabled = NO;
        self.longPressRecognizer = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(dragSourceLongPressGestureUpdated:)];
        ...
        [self.rootView addGestureRecognizer:self.longPressRecognizer];*/
    }
}
...
-(void)touchStarted{
    for(AtkDropZoneWrapper *dropZone in self.interestedDropZones)
    {
        if([dropZone.dropZone respondsToSelector:@selector(pressStarted:)])
            [dropZone.dropZone pressStarted:self];
    }
}
 ...
 */
- (void)dragSourceLongPressGestureUpdated:(UILongPressGestureRecognizer *)recognizer
{
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            switch(recognizer.state)
            {
                case UIGestureRecognizerStateBegan:
                    ...
                    if (_dragSource) {
                        [self onTouchStarted:recognizer];
                    }
                    break;
...
-(void)onTouchStarted:(UIGestureRecognizer *)recognizer{
    [self touchStarted];
}
